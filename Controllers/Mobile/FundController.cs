    ...
    public class FundController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(value.keySearch)) { filter = filter & Builders<News>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")); }
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient("fund");
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>("fundGallery");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                else
                {
                    ...
                    //if (!string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<FundCategory>.Filter.Regex("dateStart", value.startDate); }
                    //if (!string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<FundCategory>.Filter.Regex("dateEnd", value.endDate); }
