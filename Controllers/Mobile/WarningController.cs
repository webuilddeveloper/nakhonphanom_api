...
namespace mobile_api.Controllers
{
    ...
    public class WarningController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                value.statisticsCreate("warning");
                var col = new Database().MongoClient<Warning>( "warning");
                var filter = (Builders<Warning>.Filter.Eq("status", "A") & value.filterOrganization<Warning>());

                if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Warning>.Filter.Regex("code", value.code); }
                if (!string.IsNullOrEmpty(value.keySearch)) { filter = filter & Builders<Warning>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")); }
                if (!string.IsNullOrEmpty(value.category)) { filter = filter & Builders<Warning>.Filter.Eq("category", value.category); }
                if (!string.IsNullOrEmpty(value.status)) { filter = filter & Builders<Warning>.Filter.Eq("status", value.status); }
                if (!string.IsNullOrEmpty(value.description)) { filter = filter & Builders<Warning>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                if (!string.IsNullOrEmpty(value.language)) { filter = filter & Builders<Warning>.Filter.Regex("language", value.language); }
                if (value.isHighlight) { filter = filter & Builders<Warning>.Filter.Eq("isHighlight", value.isHighlight); }
                if (value.isPublic) { filter = filter & Builders<Warning>.Filter.Eq("isPublic", value.isPublic); }
                ...
                List<Warning> docs = col.Aggregate().Match(filter).SortBy(o => o.sequence).ThenByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit)
                ...
                if (!string.IsNullOrEmpty(value.code))
                {
                    ...
                    var colUpdate = new Database().MongoClient( "warning");
        ...
        public ActionResult<Response> GalleryRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Gallery>( "warningGallery");
        ...
        public ActionResult<Response> CommentRead([FromBody] Criteria value)
        {
            try
            {
                var col = new Database().MongoClient<Comment>("warningComment");
        ...
        public ActionResult<Response> CategoryRead([FromBody] Criteria value)
        {
            try
            {
                ...
                else
                {
                    ...
                    //if (!string.IsNullOrEmpty(value.startDate)) { filter = filter & Builders<WarningCategory>.Filter.Regex("dateStart", value.startDate); }
                    //if (!string.IsNullOrEmpty(value.endDate)) { filter = filter & Builders<WarningCategory>.Filter.Regex("dateEnd", value.endDate); }
