        ...
        public async System.Threading.Tasks.Task<ActionResult<Response>> CreateAsync([FromBody] Criteria value)
        {
            try
            {
                ...
                try
                {
                    ...
                    List<Employee> docs = new List<Employee>();
                    json.EmployeeOpec.ForEach(c =>
                    {
                        CertificateTeacherModel cerModel = c.certificateType.CheckCertificateType();

                        c.certificateColor = cerModel.color;
                        c.certificateStatusName = cerModel.statusName;
                        c.certificateTypeName = cerModel.certificateTypeName;
                        c.teachDegreeLevelName = c.teachDegreeLevelID.CheckTeachDegreeLevelID();
                        c.teachSubjectName = c.teachSubjectID.CheckTeachSubjectID();
                        c.fullName = c.prefix + " " + c.firstName + " " + c.lastName;
