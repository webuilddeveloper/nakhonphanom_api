...
namespace cms_api.Controllers
{
    ...
    public class RotationController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] Rotation value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "rotation");
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = Builders<Rotation>.Filter.Ne("status", "D");
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<Rotation>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<Rotation>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
                }
                else
                {
                    if (!string.IsNullOrEmpty(value.code)) { filter = filter & Builders<Rotation>.Filter.Eq("code", value.code); }
                    if (!string.IsNullOrEmpty(value.title)) { filter = filter & Builders<Rotation>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    ...
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<Rotation>.Filter.Eq("sequence", sequence); }
                ...
                var docs = col.Find(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.code, c.createDate, c.createBy, c.imageUrlCreateBy, c.updateDate, c.updateBy, c.docDate, c.docTime, c.isActive, c.sequence, c.title, c.action, c.imageUrl, c.linkUrl, c.description, c.note, c.newsPage, c.eventPage, c.privilegePage, c.mainPage, c.poiPage, c.notificationPage, c.fundPage }).ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] Rotation value)
        {
            var doc = new BsonDocument();
            try
            {
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] Rotation value)
        {
            try
            {
                var col = new Database().MongoClient( "rotation");
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "rotationGallery");
