...
namespace cms_api.Controllers
{
    ...
    public class ForceAdsController : Controller
    {
        ...
        // POST /create
        [HttpPost("create")]
        public ActionResult<Response> Create([FromBody] ForceAds value)
        {
            value.code = "".toCode();
            var doc = new BsonDocument();
            try
            {
                var col = new Database().MongoClient( "forceAds");
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                var filter = Builders<ForceAds>.Filter.Ne("status", "D");
                if (!string.IsNullOrEmpty(value.keySearch))
                {
                    filter = (filter & Builders<ForceAds>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<ForceAds>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
                }
                else
                {
                    if (!string.IsNullOrEmpty(value.code)) { filter &= Builders<ForceAds>.Filter.Eq("code", value.code); }
                    if (!string.IsNullOrEmpty(value.title)) { filter &= Builders<ForceAds>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.title), "i")); }
                    if (!string.IsNullOrEmpty(value.description)) { filter &= Builders<ForceAds>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.description), "i")); }
                    if (!string.IsNullOrEmpty(value.sequence)) { int sequence = Int32.Parse(value.sequence); filter = filter & Builders<ForceAds>.Filter.Eq("sequence", sequence); }
                ...
                var docs = col.Find(filter).SortByDescending(o => o.docDate).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.code, c.createDate, c.createBy, c.updateDate, c.updateBy, c.docDate, c.docTime, c.isActive, c.sequence, c.title, c.action, c.note, c.imageUrl, c.linkUrl, c.description, c.newsPage, c.eventPage, c.privilegePage, c.poiPage, c.notificationPage }).ToList();
        ...
        // POST /update
        [HttpPost("update")]
        public ActionResult<Response> Update([FromBody] ForceAds value)
        {
            var doc = new BsonDocument();
            try
            {
        ...
        // POST /delete
        [HttpPost("delete")]
        public ActionResult<Response> Delete([FromBody] ForceAds value)
        {
            try
            {
                var col = new Database().MongoClient( "forceAds");
        ...
        public ActionResult<Response> GalleryCreate([FromBody] Gallery value)
        {
            ...
            try
            {
                var col = new Database().MongoClient( "forceAdsGallery");
