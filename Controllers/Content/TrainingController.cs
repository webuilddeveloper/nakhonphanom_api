    ...
    public class TrainingController : Controller
    {
        ...
        public ActionResult<Response> Read([FromBody] Criteria value)
        {
            try
            {
                ...
                {
                    filter = (filter & Builders<EventCalendar>.Filter.Regex("title", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i"))) | (filter & Builders<EventCalendar>.Filter.Regex("description", new BsonRegularExpression(string.Format(".*{0}.*", value.keySearch), "i")));
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(value.action)) { filter = filter & Builders<EventCalendar>.Filter.Eq("action", value.action); }
                    if (!string.IsNullOrEmpty(value.createBy)) { filter = filter & Builders<EventCalendar>.Filter.Eq("createBy", value.createBy); }
        ...
        public ActionResult<Response> RegisterRead([FromBody] Criteria value)
        {
            try
            {
                ...
                var docs = col.Find(filter).SortBy(o => o.sequence).ThenByDescending(o => o.updateTime).ThenByDescending(o => o.updateTime).Skip(value.skip).Limit(value.limit).Project(c => new { c.reference, c.description, c.createBy, c.createDate, c.imageUrlCreateBy, c.firstName, c.lastName }).ToList();
